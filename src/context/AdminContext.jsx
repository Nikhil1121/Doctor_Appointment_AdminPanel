...
const AdminContextProvider = (props) => {
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      ...
      if (data.success) { toast.success(data.message); getAllAppointments(); getDashData(); }
      else toast.error(data.message);
    } catch (error) { toast.error(error.message); }
  };
  ...
  const confirmAppointment = async (appointmentId) => {
    try {
      const { data } = await axios.post(backendUrl + "/api/admin/confirm-appointment", { appointmentId }, { headers: { atoken: aToken } });
      if (data.success) { toast.success("Appointment Confirmed! ✅"); getAllAppointments(); getDashData(); }
      else toast.error(data.message);
    } catch (error) { toast.error(error.message); }
  };
