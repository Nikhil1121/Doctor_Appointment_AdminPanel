...
const DoctorList = () => {
  ...
  const removeDoctor = async (docId) => {
    try {
      ...
      if (data.success) {
        toast.success("Doctor Removed! Check 'Deleted Doctors' to restore 🗑️");
        setTimeout(() => {
          getAllDoctors();
          setRemovingId(null);
          setConfirmId(null);
      ...
      {doctors.length === 0 ? (
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {doctors.map((item, index) => (
            <div
              className={`border rounded-xl overflow-hidden group hover:shadow-lg transition-all duration-300 bg-white
                ...
                {confirmId === item._id ? (
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      className="flex-1 py-1.5 bg-red-500 text-white rounded-lg text-sm hover:bg-red-600 transition-all font-medium"
                    >
                      {removingId === item._id ? 'Removing...' : 'Confirm ✓'}
                    </button>
                    <button
                      onClick={() => setConfirmId(null)}
                      className="flex-1 py-1.5 border border-gray-300 text-gray-600 rounded-lg text-sm hover:bg-gray-50 transition-all"
                    >
                      Cancel
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="mt-3 w-full py-1.5 bg-red-50 text-red-500 border border-red-200 rounded-lg text-sm hover:bg-red-500 hover:text-white transition-all duration-300"
                  >
                    🗑️ Remove Doctor
                  </button>
                )}
